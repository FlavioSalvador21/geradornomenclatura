...
with st.container():
    ...
    with col2:
        copy = st.text_input(
            "2. Sigla Copy (Até 3 letras)", max_chars=3, key="copy_input"
        ).upper()
    with col3:
        editor = st.text_input(
            "3. Sigla Editor (Até 3 letras)", max_chars=3, key="editor_input"
        ).upper()

    # --- LINHA 2 (Campos 4, 5, 6) ---
    col4, col5, col6 = st.columns(3)
    with col4:
        conceito = st.text_input(
            "4. Conceito (Ex: OFFER)", key="conceito_input"
        ).upper()
    with col5:
        versao_num = st.number_input(
            "5. Número da Versão", min_value=1, step=1, key="versao"
        )
        versao_formatada = f"V{int(versao_num)}"
    with col6:
        tipo_midia = st.selectbox(
            "6. Tipo de Mídia", ["VID", "IMG", "CAR"], key="midia_input"
        )
    ...
    with col8:
        linguagem = st.selectbox(
            "8. Linguagem", ["BR", "ES"], key="lang_input"
        )
    with col9:
    ...
    mapa_anuncio = {
        ...
        "ITERAÇÕES": "ITE",
        "MODELAGEM DE ORGÂNICO": "ORG",  # <-- NOVA OPÇÃO ADICIONADA
    }
    tipo_anuncio_nome = st.selectbox(
        "10. Tipo de Anúncio", list(mapa_anuncio.keys()), key="tipo_anuncio"
    )
    tipo_anuncio_sigla = mapa_anuncio[tipo_anuncio_nome]

    # --- LINHA 5 (Campo 11) ---
    descricao = st.text_input(
        "11. Descrição (Texto Livre)",
...
if gerar:
    if not copy or not editor or not conceito:
        st.error(
            "⚠️ Por favor, preencha os campos de Copy, Editor e Conceito."
        )
    else:
        ...
        resultado_final = (
            f"{resultado_base} - {descricao}" if descricao else resultado_base
        )
        ...
        if len(resultado_final) > 100:
            st.warning(
                "Nota: O nome está bem longo. Verifique se o destino aceita esse comprimento."
            )
